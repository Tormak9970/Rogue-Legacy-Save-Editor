...
<script lang="ts">
  ...
  import FaderModal from "../../components/modals/FaderModal.svelte";
  ...
  let showFader = $showingAbout || $showingBackup;

  let showingAboutUnsub:Unsubscriber;
  let showingBackupUnsub:Unsubscriber;

  async function loadSave(e: Event) {
    const path = (e.currentTarget as HTMLInputElement).value;
    AppController.log(
      `Updated save dir for Rogue Legacy ${$seriesEntry + 1} to ${path}`
    );
    $saveDirPath = path;
  }
  function setProfile(value: string) {
    AppController.log(
      `Updated selected profile for Rogue Legacy ${
        $seriesEntry + 1
      } to ${value}`
    );
  ...
  async function confirmReload(e: Event) {
  ...
  async function makeBackup(e: Event) {
    await AppController.backup();
  }
  function loadBackup(e: Event) {
    AppController.showBackupWindow();
  }
  function showAboutModal(e: Event) {
    AppController.showAboutWindow();
  }

  function switchGameVersion(e: Event) {
    AppController.switchGameVersion();
  }
  ...
  onMount(async () => {
    ...
    showingBackupUnsub = showingBackup.subscribe((newVal) => {
      showFader = $showingAbout || newVal;
    });
  });

  onDestroy(() => {
    if (showingAboutUnsub) showingAboutUnsub();
    if (showingBackupUnsub) showingBackupUnsub();
...
<main>
  ...
  <ConfirmModal
    ...
    message={"Are you sure you want to reload? You will loose your changes."}
    onConfirm={reload}
    onCancel={async () => {
      $showConfirmReload = false;
    }} />
  <FaderModal show={showFader} />
  <div class="content">
