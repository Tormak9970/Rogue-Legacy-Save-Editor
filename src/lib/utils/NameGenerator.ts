...
export class NameGenerator {
  ...
  constructor() {
    ...
    const invRegex = new RegExp(
      'inventory_item:\\s?\\.type\\s?"([a-z_]*)"\\s*\\.id\\s*"([a-z_]*)".*'
    );
    const inventoryParser: Parser = {
      async parseFile(fPath: string, names: Set<string>) {
        // split lines
        const lines = Buffer.from(await fs.readBinaryFile(fPath))
          .toString()
          .split("\r\n");
    ...
    this.addParser("quest.plot_quests.json", {
      async parseFile(fPath: string, names: Set<string>) {
        const buf = await fs.readBinaryFile(fPath);
        NameGenerator.addSimpleJSONArrayEntryIDs(
          buf,
          "plot_quests",
          "id",
          names
   ...
   */
  static addSimpleJSONArrayEntryIDs(
    data: Uint8Array,
    arrayName: string,
    idString: string,
    set: Set<string>
