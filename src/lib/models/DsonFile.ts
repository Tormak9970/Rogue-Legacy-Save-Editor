...
export class DsonHeader {
  ...
  parse(reader: Reader) {
    ...
    if (this.magicNr != MAGIC_NUMBER)
      throw new Error(
        `Expected magic number to be ${MAGIC_NUMBER} but was ${this.magicNr}`
      );
    ...
    if (this.headerLength != HEADER_SIZE)
      throw new Error(
        `Expected header length to be 64 but was ${this.headerLength}`
      );
  ...
  findNextSmallestOffset(offset: number): number {
    ...
    for (let i = 0; i < this.entries.length; i++) {
      if (
        ...
        (bestIdx == -1 || this.entries[i].offset < bestOffset)
      ) {
        bestIdx = i;
...
export class DsonMeta2BlockEntry {
  ...
  parse(reader: Reader) {
    ...
    this.meta1BlockIdx =
      (this.fieldInfo & 0b1111111111111111111100000000000) >> 11;
  }
}
...
export class DsonData {
  ...
  parse(reader: Reader, dson: DsonFile, unhashBehavior: UnhashBehavior) {
    ...
    for (let i = 0; i < dson.meta2Block.entries.length; i++) {
      ...
      field.name = DsonData.readName(
        reader,
        meta2Entry.offset,
        meta2Entry.nameLen - 1
      );
      ...
      if (meta2Entry.isObject) {
        ...
        field.setNumChildren(
          dson.meta1Block.entries[meta2Entry.meta1BlockIdx].numDirectChildren
        );

        if (
          dson.meta1Block.entries[meta2Entry.meta1BlockIdx].parentIdx !=
          parentIdxStack.peek()
        ) {
      ...
      while (
        !fieldStack.isEmpty() &&
        JSON.stringify(fieldStack.peek().type) ==
          JSON.stringify(FieldType.TYPE_OBJECT) &&
        fieldStack.peek().hasAllChildren()
      ) {
        fieldStack.pop();
        parentIdxStack.pop();
      }
    ...
    this.data.parse(
      new Reader(
        reader.data.slice(
          ...
          this.header.dataOffset + this.header.dataLength
        )
      ),
      this,
      behavior
